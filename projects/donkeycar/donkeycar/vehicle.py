...
from .parts.object_detector.green_traffic_detector import GreenTrafficDetector
...
class Vehicle:
    ...
    def update_parts(self):
        ...
        for entry in self.parts:
            ...
            if self.go == False and self.mem.get(['go'])[0]:
                self.go = True    
            ...
            if run:
                ...
                p = entry['part']
                if self.go == False: 
                    if isinstance(p, GreenTrafficDetector) or isinstance(p, PiCamera):        
                        self.process_part(p, entry)
                else:
                    if not isinstance(p, GreenTrafficDetector):
                        self.process_part(p, entry)

    # process parts
    def process_part(self, p, entry):
        print("Process ", p.__class__.__name__)
